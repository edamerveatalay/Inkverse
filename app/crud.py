...
async def get_user_by_email (session: AsyncSession, email: str ) : #read işlemi okuma işlemi yapıyor. 
...
async def create_user (session: AsyncSession, user_data: UserCreate ): #usercreate sınıfından verileri aldık (email, password)
...
async def get_all_users (session: AsyncSession): 
  result = await session.execute (select(User))#Bu komut, veritabanına gerçek sorguyu gönderir.
  ...
  users = result.scalars().all()#.scalars() bu satırları sadece model nesnelerine çevirir.
  return users
